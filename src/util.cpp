...
const char * const BITCOIN_CONF_FILENAME = "swap.conf";
const char * const BITCOIN_PID_FILENAME = "swapd.pid";
...
bool LogAcceptCategory(const char* category)
{
    if (category != NULL)
    {
        ...
        if (ptrCategory.get() == NULL)
        {
            ...
            // "swap" is a composite category enabling all Swap-related debug output
            if(ptrCategory->count(string("swap"))) {
...
boost::filesystem::path GetDefaultDataDir()
{
    ...
    // Windows < Vista: C:\Documents and Settings\Username\Application Data\SwapCore
    // Windows >= Vista: C:\Users\Username\AppData\Roaming\SwapCore
    // Mac: ~/Library/Application Support/SwapCore
    // Unix: ~/.swapcore
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "SwapCore";
#else
    fs::path pathRet;
...
#ifdef MAC_OSX
    // Mac
    return pathRet / "Library/Application Support/SwapCore";
#else
    // Unix
    return pathRet / ".swapcore";
#endif
#endif
}
                    ...
                    map<string, vector<string> >& mapMultiSettingsRet)
{
    ...
    if (!streamConfig.good()){
        // Create empty swap.conf if it does not excist
    ...
    {
        // Don't overwrite existing settings so command line settings override swap.conf
